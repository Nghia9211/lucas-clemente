...
func (s *session) handleAckFrame(frame *wire.AckFrame) error {
	...
	// if err == nil && pth.pathID != protocol.InitialPathID && s.scheduler.SchedulerName == "sac-cc" {
	// 	s.scheduler.GetStateAndRewardSACcc(s, pth)
	// }
	...
	if err == nil && pth.pathID != protocol.InitialPathID && s.scheduler.SchedulerName == "sac-cc" {
			isLoss := false
			_, _, newLost := pth.sentPacketHandler.GetStatistics()
			// fmt.Println("Old lost : ", oldLost)
			// fmt.Println("New lost: ", newLost)
			if oldLost != newLost {
				
				isLoss = true
				// fmt.Println("LOSTTTTT : ", isLoss )
			}
			// fmt.Println("GeetSTATEREWARD ", isLoss )
			s.scheduler.GetStateAndRewardSACcc(s, pth, isLoss)		
	}
