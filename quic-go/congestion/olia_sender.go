...
func (o *OliaSender) SignalSAC(factor float64) {
	...
	// 	fmt.Println("Max CWWND : ", o.congestionWindow)
	// }

	newCwnd := float64(o.congestionWindow) * factor
	...
	// Nếu muốn tăng CWND khi factor > 1 mà CWND bị kẹt ở 1, đảm bảo tăng lên 2
	if (factor > 1 && float64(o.congestionWindow) == 1) {
		newCwnd = 2
	}

	// Gán giá trị sau khi giới hạn max
	o.congestionWindow = utils.MinPacketNumber(
