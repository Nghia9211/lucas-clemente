...
// Define State for SAC-Congestion Control
type StateSACcc struct {
	CWND float64 `json:"CWND"`
	INP  float64 `json:"INP"`
	SRTT float64 `json:"SRTT"`
	VRTT float64 `json:"VRTT"`
}

// Payload Structure
type RewardPayloadSACcc struct {
	State       StateSACcc  `json:"state"`
	NextState   StateSACcc  `json:"next_state"`
	Action      ActionSACCC `json:"action"`
	Reward      float64     `json:"reward"`
	Done        bool        `json:"done"`
	ModelID     uint64      `json:"model_id"`
	CountReward uint16      `json:"count_reward"`
